 ...
 */
public class ZelCashZelNodeDialog extends ZelCashJDialog {
  ...
  public ZelCashZelNodeDialog(
      ZCashUI parent, final ZCashClientCaller clientCaller,
      final ZCashInstallationObserver installationObserver, String aliasToEdit,
      final LabelStorage labelStorage) throws IOException {
    parentFrame = parent;
    ...
    ZelCashJLabel infoLabel =
        new ZelCashJLabel(langUtil.getString("dialog.zelcashnewzelnode.info"));
    ...
    addFormField(detailsPanel,
                 langUtil.getString("dialog.zelcashnewzelnode.name"),
                 zelNodeName = new ZelCashJTextField(50));
    addFormField(detailsPanel,
                 langUtil.getString("dialog.zelcashnewzelnode.ip"),
                 zelNodeIP = new ZelCashJTextField(50));
    addFormField(detailsPanel,
                 langUtil.getString("dialog.zelcashnewzelnode.key"),
                 zelNodeKey = new ZelCashJTextField(50));
    addFormField(detailsPanel,
                 langUtil.getString("dialog.zelcashnewzelnode.output"),
                 zelNodeOutput = new ZelCashJComboBox<String>());
    addFormField(detailsPanel,
                 langUtil.getString("dialog.zelcashnewzelnode.output.txid"),
                 zelNodeOutputTxid = new ZelCashJTextField(50));
    addFormField(detailsPanel,
                 langUtil.getString("dialog.zelcashnewzelnode.output.index"),
                 zelNodeOutputindex = new ZelCashJTextField(50));
    addFormField(detailsPanel,
                 langUtil.getString("dialog.zelcashnewzelnode.address"),
                 zelNodeAddress = new ZelCashJTextField(50));
    addFormField(detailsPanel,
                 langUtil.getString("dialog.zelcashnewzelnode.amount"),
                 zelNodeAmount = new ZelCashJTextField(50));
        ...
        while ((st = br.readLine()) != null) {
          ...
          if (st.startsWith("#") || emptyLine.equals("")) {
            ...
            if (zelNodeInfo[0].equals(this.aliastoEdit)) {
              ...
              try {
                JsonObject txinfo =
                    clientCaller.getTransactionInfo(zelNodeInfo[3]);
                ...
                for (int i = 0; i < details.size(); ++i) {
                  ...
                  category =
                      obj.get("category").toString().replaceAll("[\n\r\"]", "");
                  if (vout.equals(zelNodeInfo[4]) && "send".equals(category)) {
                    detailAmount = obj.get("amount")
                                       .toString()
                                       .replaceAll("[\n\r\"]", "")
                                       .substring(1);
                ...
                if (!addressFound) {
                  for (int i = 0; i < details.size(); ++i) {
                    JsonObject obj = details.get(i).asObject();
                    vout =
                        obj.get("vout").toString().replaceAll("[\n\r\"]", "");
                    category = obj.get("category")
                                   .toString()
                                   .replaceAll("[\n\r\"]", "");

                    if (vout.equals(zelNodeInfo[4]) &&
                        "receive".equals(category)) {
              ...
              } catch (WalletCallException | IOException |
                       InterruptedException e) {
                Log.error("Error calling getRawTransactionDetails:" +
                          e.getMessage());
              }
              break;
            }
    ...
    ZelCashJButton closeButon = new ZelCashJButton(
        langUtil.getString("dialog.about.button.close.text"));
    closePanel.add(closeButon);
    saveButton =
        new ZelCashJButton(langUtil.getString("dialog.zelcashuiedit.save"));
    ...
    saveButton.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        if ("".equals(zelNodeName.getText().replaceAll(" ", "")) ||
            ...
            zelNodeOutput == null || zelNodeOutput.getItemCount() == 0 ||
            langUtil.getString("dialog.zelcashnewzelnode.select.output")
                .equals(zelNodeOutput.getSelectedItem().toString())) {
          JOptionPane.showMessageDialog(
              null,
              LanguageUtil.instance().getString(
                  "dialog.zelcashnewzelnode.fields.missing"),
              LanguageUtil.instance().getString(
                  "dialog.zelcashnewzelnode.fields.missing.title"),
        ...
        if (!zelNodeName.getText().matches("\\w+")) {
          JOptionPane.showMessageDialog(
              null,
              LanguageUtil.instance().getString(
                  "dialog.zelcashnewzelnode.fields.alias.wrong"),
              LanguageUtil.instance().getString(
        ...
        if (!zelNodeIP.getText().endsWith(":16125") &&
            !zelNodeIP.getText().endsWith(":26125")) {
          JOptionPane.showMessageDialog(
              null,
              LanguageUtil.instance().getString(
                  "dialog.zelcashnewzelnode.fields.ip.wrong"),
              LanguageUtil.instance().getString(
                  "dialog.zelcashnewzelnode.fields.error.adding.title"),
              JOptionPane.ERROR_MESSAGE);
          return;
      ...
      public void itemStateChanged(ItemEvent event) {
        if (event.getStateChange() == ItemEvent.SELECTED) {
          ...
          if (source instanceof ZelCashJComboBox) {
            ...
            if (index != 0) {
              zelNodeAmount.setText(
                  langUtil.getString("zelnodespanel.zelnodes.button.loading"));
              zelNodeAddress.setText(
                  langUtil.getString("zelnodespanel.zelnodes.button.loading"));
              zelNodeOutputindex.setText(
                  langUtil.getString("zelnodespanel.zelnodes.button.loading"));
              zelNodeOutputTxid.setText(
                  langUtil.getString("zelnodespanel.zelnodes.button.loading"));
              ...
              try {
                ...
                JsonObject txinfo =
                    clientCaller.getTransactionInfo(outputinfo[0]);
                JsonArray details = txinfo.get("details").asArray();
                String vout;
                String category;
                String detailAmount = "0";
  ...
  private void gelZelNodeKey() {
    ...
    try {
      zelNodeKey.setText(
          clientCaller.getZelNodeKey().replaceAll("[\n\r\"]", ""));
  ...
  private void getZelNodeOutputs() {
    ...
    try {
      ...
      zelNodeOutput.addItem(
          this.langUtil.getString("dialog.zelcashnewzelnode.select.output"));
      for (int i = 0; i < ja.size(); ++i) {
        JsonObject obj = ja.get(i).asObject();
        String txhash = obj.get("txhash").toString().replaceAll("[\n\r\"]", "");
        String outputidx =
            obj.get("outputidx").toString().replaceAll("[\n\r\"]", "");
  ...
  private void addFormField(ZelCashJPanel detailsPanel, String name,
                            JComponent field, ZelCashJButton button) {
    ZelCashJPanel tempPanel =
        new ZelCashJPanel(new FlowLayout(FlowLayout.LEFT, 4, 2));
    ZelCashJLabel tempLabel = new ZelCashJLabel(name, JLabel.RIGHT);
    // TODO: hard sizing of labels may not scale!
    final int width = new ZelCashJLabel("Sender identification T address:")
                          .getPreferredSize()
                          .width +
                      10;
    tempLabel.setPreferredSize(
        new Dimension(width, tempLabel.getPreferredSize().height));
  ...
  private void saveSettings() {
    ...
    try {
          ...
          if (!line.startsWith("#")) {
            ...
            if (output.equals(zelNodeOutput.getSelectedItem().toString())) {
              JOptionPane.showMessageDialog(
                  null,
                  LanguageUtil.instance().getString(
                      "dialog.zelcashnewzelnode.fields.output.duplicated"),
                  LanguageUtil.instance().getString(
                      "dialog.zelcashnewzelnode.fields.error.adding.title"),
                  JOptionPane.ERROR_MESSAGE);
              return;
            ...
            if (alias.toLowerCase().equals(zelNodeName.getText()
                                               .toLowerCase()
                                               .replaceAll(" ", "")
                                               .replaceAll("[\n\r\"]", ""))) {
              JOptionPane.showMessageDialog(
                  null,
                  LanguageUtil.instance().getString(
                      "dialog.zelcashnewzelnode.fields.alias.duplicated"),
                  LanguageUtil.instance().getString(
                      "dialog.zelcashnewzelnode.fields.error.adding.title"),
                  JOptionPane.ERROR_MESSAGE);
              return;
            ...
            if (ip.toLowerCase().equals(zelNodeIP.getText()
                                            .toLowerCase()
                                            .replaceAll(" ", "")
                                            .replaceAll("[\n\r\"]", ""))) {
              JOptionPane.showMessageDialog(
                  null,
                  LanguageUtil.instance().getString(
                      "dialog.zelcashnewzelnode.fields.ip.duplicated"),
                  LanguageUtil.instance().getString(
                      "dialog.zelcashnewzelnode.fields.error.adding.title"),
                  JOptionPane.ERROR_MESSAGE);
              return;
            ...
            if (zelnodekey.equals(zelNodeKey.getText())) {
              JOptionPane.showMessageDialog(
                  null,
                  LanguageUtil.instance().getString(
                      "dialog.zelcashnewzelnode.fields.key.duplicated"),
                  LanguageUtil.instance().getString(
                      "dialog.zelcashnewzelnode.fields.error.adding.title"),
                  JOptionPane.ERROR_MESSAGE);
              return;
      ...
      Log.info(
          "Now adding info needed to run zelNode to zelcash conf file if needed");
      File zelcashConf =
      ...
      try {
        fw = new FileWriter(zelnodeConf,
                            true); // the true will append the new data
        fw.write(
            zelNodeName.getText().replaceAll(" ", "").replaceAll("[\n\r\"]",
                                                                 "") +
            " " +
            zelNodeIP.getText().replaceAll(" ", "").replaceAll("[\n\r\"]", "") +
            " " + zelNodeKey.getText() + " " +
            zelNodeOutput.getSelectedItem().toString() +
            System.getProperty(
                "line.separator")); // appends the string to the file
      ...
      if (initialInstallationObserver.isOnTestNet()) {
        ...
        JOptionPane.showMessageDialog(
            null,
            LanguageUtil.instance().getString(
                "wallet.zelnodes.restart.testnet.message"),
            LanguageUtil.instance().getString("wallet.zelnodes.restart.title"),
            JOptionPane.INFORMATION_MESSAGE);
      } else {
        initialInstallationObserver = null;
        Object[] options = {
            LanguageUtil.instance().getString("ipfs.wrapper.options.yes"),
            LanguageUtil.instance().getString("ipfs.wrapper.options.no")};
        if (daemonNeedsToBeReindexed) {
          int option = JOptionPane.showOptionDialog(
              null,
              LanguageUtil.instance().getString(
                  "wallet.zelnodes.restart.reindex.message"),
              LanguageUtil.instance().getString(
                  "wallet.zelnodes.restart.title"),
          ...
          int option = JOptionPane.showOptionDialog(
              null,
              LanguageUtil.instance().getString(
                  "wallet.zelnodes.restart.message"),
              LanguageUtil.instance().getString(
                  "wallet.zelnodes.restart.title"),
              JOptionPane.DEFAULT_OPTION, JOptionPane.INFORMATION_MESSAGE, null,
              options, options[0]);
  ...
  public static void removeZelNode(String zelNodeAlias) {
    ...
    try {
      ...
      Log.error("Error deleting zelnode " + zelNodeAlias + ":" +
                ex.getMessage());
    } finally {
      Log.info("Zelnode removed from configuration file. Alias removed:" +
               zelNodeAlias);
    }
  }
  ...
  public static void removeEmptyLinesFromNodesConfigurationFile() {
    try {
      ...
      Log.error("Error deleting empty lines from zelnodes.conf:" +
                ex.getMessage());
    }
  }
}
